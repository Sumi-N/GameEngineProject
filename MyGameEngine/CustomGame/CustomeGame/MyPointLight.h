...
inline void MyPointLight::Update(float i_dt)
{
	...
	if (UserInput.QueryKey(VirtualKey::KEY_I) == InputState::Pressed)
	{
		Translate(Vec3f(pos.x, pos.y + 1.0f, pos.z));
	}

	if (UserInput.QueryKey(VirtualKey::KEY_K) == InputState::Pressed)
	{
		Translate(Vec3f(pos.x, pos.y - 1.0f, pos.z));
	}

	if (UserInput.QueryKey(VirtualKey::KEY_J) == InputState::Pressed)
	{
		Translate(Vec3f(pos.x + 1.0f, pos.y, pos.z));
	}

	if (UserInput.QueryKey(VirtualKey::KEY_L) == InputState::Pressed)
	{
		Translate(Vec3f(pos.x - 1.0f, pos.y, pos.z));
	}
