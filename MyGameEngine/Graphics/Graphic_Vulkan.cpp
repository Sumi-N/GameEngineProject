...
#include <GraphicAPI/stb/stb_image.h>
...
void HelloTriangleApplication::queryExtensions()
{
...
void HelloTriangleApplication::createInstance()
{
	...
	{
		throw std::runtime_error("validation layers requested, bu not available!");
	}
...
bool HelloTriangleApplication::isDeviceSuitable(VkPhysicalDevice device)
{
	...
	{
		SwapChainsSupportDetails swapChainSupport = querySwapChainSupport(device);
		swapChainAdequate = !swapChainSupport.formats.empty() && !swapChainSupport.presentModes.empty();
	}

	return indices.isComplex() && extensionsSupported && swapChainAdequate && supportedFeatures.samplerAnisotropy;
}
...
{
	SwapChainsSupportDetails swapChainSupport = querySwapChainSupport(physicalDevice);
...
HelloTriangleApplication::SwapChainsSupportDetails HelloTriangleApplication::querySwapChainSupport(VkPhysicalDevice device)
{
	SwapChainsSupportDetails details;
...
void HelloTriangleApplication::createSyncObjects()
{
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
	{
		if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAvailableSemaphore[i]) != VK_SUCCESS || vkCreateSemaphore(device, &semaphoreInfo, nullptr, &renderFinishedSemaphore[i]) != VK_SUCCESS)
		{
...
void HelloTriangleApplication::createTextureImage()
{
	...
	//stbi_uc* pixels = stbi_load("C:\\Users\\sumil\\source\\repos\\GameEngineProject\\MyGameEngine\\Assets\\textures\\texture.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
...
void HelloTriangleApplication::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
{
	...
	vkCmdCopyBufferToImage(commandBuffers, buffer, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
