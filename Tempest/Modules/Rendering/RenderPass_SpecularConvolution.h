...
namespace Tempest
{
	...
	public:
		...
		{
			ShaderInfo::Load(PATH_SUFFIX BIN_SHADER_PATH "cubemap_specular_convolution.ts", shader);
...
#error
			...
			// Workaround for frame buffer miplevel issue
			{
				cubemap_texture.image_views.Clear();
				VkImageViewCreateInfo view_info{};
				for (uint32_t i = 0; i < cubemap_texture_info.mip_count; i++)
				{
					view_info.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
					view_info.image = cubemap_texture.image;
					view_info.viewType = VK_IMAGE_VIEW_TYPE_CUBE;
					view_info.format = cubemap_texture.format;
					view_info.subresourceRange.aspectMask = cubemap_texture.aspect;
					view_info.subresourceRange.baseMipLevel = i;
					view_info.subresourceRange.levelCount = 1;
					view_info.subresourceRange.baseArrayLayer = 0;
					view_info.subresourceRange.layerCount = cubemap_texture_info.count;

					VkImageView image_view;
					auto image_view_create_result = vkCreateImageView(i_device.logical_device, &view_info, nullptr, &image_view);
					DEBUG_ASSERT(image_view_create_result == VK_SUCCESS);

					cubemap_texture.image_views.PushBack(image_view);
				}
			}
