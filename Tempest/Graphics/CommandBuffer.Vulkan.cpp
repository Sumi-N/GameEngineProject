...
namespace Tempest
{
	void CommandBuffer::Initialize(Device& i_device)
	{
		...
		// Create CommandPool
		{
			...
			if (vkCreateCommandPool(device->logical_device, &pool_create_info, nullptr, &commandpool) != VK_SUCCESS)
			{
				DEBUG_ASSERT(false);
			}
			...
			if (vkAllocateCommandBuffers(device->logical_device, &commandbuffer_allocate_info, commandbuffers) != VK_SUCCESS)
			{
				DEBUG_ASSERT(false);
			}
		}
